defmodule Minesweeper do
  ...
  defp process_board(board) do
    ...
    bombs = map |> Enum.filter(fn({_key, v}) -> v == "*" end)
    map = bombs |> annotated_map(map)

    map |> map_to_board(length(board))
  end

  defp board_to_map(board),
    do: board 
          |> Enum.with_index 
          |> Enum.reduce(%{},
              fn({row, indX}, acc) -> 
  ...
  defp map_to_board(map, board_length),
    do: Enum.reduce(
          Enum.to_list(0..board_length - 1),
          [],
          fn(ind, acc) -> acc ++ [create_row_from_map(map, ind)] end)

  defp create_row_from_map(map, ind),
    do: map |> Enum.filter(fn({{x, _y}, _v}) -> x == ind end)
